...
export class SessionSetupComponent implements OnInit {
  ...
  ngOnInit() {
    this._http.get("http://localhost:9090/api/public/themes").subscribe(data => this.themes = data.json());
  ...
  addTheme() {
    ...
    this._http.post("http://localhost:9090/api/public/themes", theme).subscribe(theme => {
